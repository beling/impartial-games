//! Endgame database slices that use the perfect hashing provided by the `cmph` library
//! (the CHD algorithm; see <https://cmph.sourceforge.net/>).
//!
//! The main type is [`ClusterCMPH`], which stores a map from positions (of the type `u32`)
//! to nimbers: the (packed) minimal perfect hash of the positions and the array of nimbers
//! addressed by the hash values.
...
// The structures and functions below mirror (as generated by bindgen) the C API of the `cmph` library.

/// Opaque configuration object of the `cmph` library.
#[repr(C)]
...
/// Opaque minimal perfect hash object of the `cmph` library.
#[repr(C)]
#[derive(Debug, Copy, Clone)]
struct __cmph_t {
    _unused: [u8; 0],
...
/// Adapter that provides the `cmph` library with sequential access to the keys.
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
/// which uses the minimal perfect hash built by the CHD algorithm of the `cmph` library.
pub struct ClusterCMPH {
    /// The packed minimal perfect hash function.
    packed_hash: Box<[u8]>,
    /// The nimbers of the (hashed) positions, packed on `bits_per_value` bits each.
    nimbers: Box<[u64]>,
    /// The number of bits used to store each nimber.
    bits_per_value: u8
}

impl ClusterCMPH {
    /// Returns the (perfect hash) index assigned by the packed hash to the given position `p`.
    ...
    /// Constructs the cluster from the given (sorted) arrays of keys and values,
    /// using `bits_per_value` bits to store each value,
    /// and `lambda` (an average number of keys per bucket, tunable to obtain
    /// different trade-offs between generation time and representation size)
    /// to build the perfect hash.
    ...
    /// Constructs the cluster from the given (sorted) arrays of keys and values,
    /// using the minimal number of bits sufficient to store the largest value
    /// and `lambda` (an average number of keys per bucket) to build the perfect hash.
    #[inline]
